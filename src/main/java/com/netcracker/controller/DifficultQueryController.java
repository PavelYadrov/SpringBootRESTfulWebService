...
public class DifficultQueryController {
    ...
    public ResponseEntity<List<PurchaseViewGreaterSum>> getAllPurchaseInfoGreaterThanSum(@PathVariable(name = "sum") Double sum){
    ...
    public ResponseEntity<List<PurchaseViewNameLocDate>> findAllPurchasesAfterMounthAndSameLocs(@PathVariable(name = "month") Long month){
    ...
    public ResponseEntity<List<ShopViewWithParams>> getAllShopsOmitMonthWithDiscountInterval(@PathVariable(name = "location") String loc,
    ...
    public ResponseEntity<List<BookViewWithQuantity>> getAllBooksByQuantityBuyPlaced(@PathVariable(name = "quantity")Integer quantity){
        if(quantity<=0){
